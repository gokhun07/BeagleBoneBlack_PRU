...
Input channel is P9_28. Signal measuring this channel calculated as period and freq.

first it must be enabled  PRU0 and than p31.3 pin activated in overlay.After that overlay must be 
...
int main(void)
  {
     ...
     if(getuid()!=0){
       ...
       prussdrv_map_prumem(PRUSS0_PRU0_DATARAM,(void **)&pruRam); // Local Ram 
